...
const login = async (req, res) => {
    try {
        ...
        const userObject = await Utilisateurs.findOne({
            ...
            attributes: ['ID_UTILISATEUR', 'PASSWORD', 'ID_PROFIL', 'TELEPHONE', 'EMAIL', 'NOM', 'PRENOM', 'IS_ACTIF', 'IMAGE','ID_CORP_CORPORATE', "ID_RIDER"],
            include:[
                {model:Profil,
                as:"profil",
...
const getNewAccessToken = async (req, res) => {
    try {
        ...
        jwt.verify(refreshToken, process.env.JWT_REFRESH_PRIVATE_KEY || DEFAULT_JWT_REFRESH_PRIVATE_KEY, async (error) => {
            try {
                if (error) {
...
const findUserInfo = async (req, res) => {
    try {
        const userObject = await Utilisateurs.findOne({
            ...
            attributes: ['ID_UTILISATEUR', 'ID_PROFIL', 'TELEPHONE', 'EMAIL', 'NOM', 'PRENOM', 'IS_ACTIF', 'IMAGE','ID_CORP_CORPORATE'],
            include: [
              {
                model: Profil,
                as: "profil",
      ...
      if (userObject) {
          const user = userObject.toJSON()
            ...
            //         code = generateCode(4)
                ...
                var subject = req.__('auth_riders.controller.getDataCompteoublier.title')
