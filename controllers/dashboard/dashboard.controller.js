...
const getDriverTodayPerformacne = async (req, res) => {
          try {
                    ...
                    `
                    const performance = (await query(sqlQuery, [req.userId, [IDS_COURSE_STATUS.COURSE_TERMINE, IDS_COURSE_STATUS.TERMINE_PAR_ADMIN], moment(new Date(today)).tz("Africa/Bujumbura").get('date'), moment(new Date(today)).tz("Africa/Bujumbura").get('month') + 1, moment(new Date(today)).tz("Africa/Bujumbura").get('year')]))[0]
...
const getTripsByCategory = async (req, res) => {
          try {
                    ...
                    const startDateFormat = moment.tz(startDate, req.headers.timezone || DEFAULT_TIMEZONE).set("hours", 0).set("minutes", 0).set("seconds", 0).utc().format("YYYY-MM-DD HH:mm:ss")
                    const endDateFormat = endDate ?
                      moment.tz(endDate, req.headers.timezone || DEFAULT_TIMEZONE).set("hours", 23).set("minutes", 59).set("seconds", 59).utc().format("YYYY-MM-DD HH:mm:ss") :
                      moment.tz(new Date(), req.headers.timezone || DEFAULT_TIMEZONE).set("hours", 23).set("minutes", 59).set("seconds", 59).utc().format("YYYY-MM-DD HH:mm:ss")
                    ...
                    const tripsBySlots = slots.map(slot => {
                              const start = moment.tz(startDate ? startDate : new Date(), req.headers.timezone || DEFAULT_TIMEZONE).set("hours", slot.start[0]).set("minutes", slot.start[1]).set("seconds", 0).utc()
                              const end = moment.tz(startDate ? startDate : new Date(), req.headers.timezone || DEFAULT_TIMEZONE).set("hours", slot.end[0]).set("minutes", slot.end[1]).set("seconds", 0).utc()
